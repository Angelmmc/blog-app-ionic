 ...
 <ion-page>
    ...
    <ion-content :fullscreen="true">
      <div class="custom-title-container">
        <h1 class="custom-title">Guardados<strong class="title-dot">.</strong></h1><br>
      </div>
      ...
      <div v-if="loading" class="loading-container">
        ...
        <p>Cargando tus blogs guardados...</p>
      </div>
      <!-- Empty state -->
      <div v-else-if="!loading && blogsGuardados.length === 0" class="empty-state">
        <ion-icon :icon="bookmarkOutline" size="large"></ion-icon>
        <h2>No tienes blogs guardados</h2>
        <p>Los blogs que guardes aparecerán aquí para que puedas acceder a ellos fácilmente.</p>
      </div>
      <!-- Blogs guardados -->
...
const fetchBlogsGuardados = async () => {
  try {
    ...
    const response = await fetch("/data/saved.json");
    const data = await response.json();
    
    // Obtener los blogs de la respuesta
    blogsGuardados.value = data.data;
